...
import { NonEmptyStringSchema, CountSchema, IdSchema } from '@/utils/server/base-schemas'
  ...
  getById: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      return ctx.db.connection().execute(async (conn) => {
         ...
         */
        const totalUserFriendCountSub = conn
        ...
        .groupBy('userId')
        .as('userTotalFriendCount')

      const userRes = await conn
        .selectFrom('users as friends')
        .innerJoin('friendships', 'friendships.friendUserId', 'friends.id')
        .innerJoin(totalUserFriendCountSub, 'userTotalFriendCount.userId', 'friends.id')
      ...
      const userResmutualFriendCountRes = await conn
      ...
      if (!userResmutualFriendCountRes) {
        throw new TRPCError({ code: 'NOT_FOUND' })
      }

      return {
        ...userRes,
        mutualFriendCount: userResmutualFriendCountRes.mutualFriendCount,
      }
